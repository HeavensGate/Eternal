/obj/structure/coffin_dispenser
	...
	desc = "This machine seems to pneumatically release coffins at the press of a button, and fashion wood into new coffins. It menaces with splinters waiting to happen."
	icon_state = "cd0"
	anchored = 1
	density = 0
	var/numofcoffins = 0
...
/obj/structure/Rollerbed_dispenser/attack_robot(mob/user as mob)
	if(Adjacent(user))
		return src.attack_hand(user)
	else
		user << "<span class = \"warning\">You attempt to interface with the control circuits but find they are not connected to your network.  Maybe in a future firmware update.</span>"
