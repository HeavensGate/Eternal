	...
	if (M.a_intent == "help")
		...
		if (M.a_intent == "hurt")
			...
			if ((prob(75) && health > 0))
				...
				M.attack_log += text("\[[time_stamp()]\] <font color='red'>[pick(attack.attack_verb)] [src.name] ([src.ckey])</font>")
				src.attack_log += text("\[[time_stamp()]\] <font color='orange'>Has been [pick(attack.attack_verb)] by [M.name] ([M.ckey])</font>")
